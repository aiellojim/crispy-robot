...
| `SiteChatEbConsolePanel` | ~2615–2870（`CustomerAccessPanel` 之後、`JiraTab` 之前） | 2026-08-27 新增，2026-09-01 改成工作佇列架構（詳見 `docs/todo.md` #11）：內部限定的 SiteChat→eb-console 推送面板（預覽 `sitechat_settings` 的三語 bot_name/greeting/theme + 人工審核）。「確認推送」`handlePush()` 現在只是對 `sitechat_ebconsole_pushes` 做一筆 `insert(status:'pending')`（**不再呼叫** `EBCONSOLE_PROXY`/`ebconsole-proxy`，那支 Edge Function 目前保留部署但沒有呼叫者），真正打 eb-admin.aiello.ai 的工作由 `scripts/ebconsole-push-agent.mjs`（Jim 本機跑的常駐輪詢腳本，因為 eb-admin 的防火牆只讓內網連、Edge Function 沒有固定出口 IP 連不到）認領處理；面板在有 pending/processing 任務時每 3 秒自動輪詢刷新紀錄。觸發按鈕是 `ProjectDetail` 裡緊接在「飯店填寫表單連結」卡片後面、`info.products.includes("SiteChat")` 才顯示的新 Card；狀態 `showEbConsolePush` 宣告處緊鄰既有的 `showCustomerAccess`。 |
