...
- **2026-08-07 追加：Amenity/Repair「上傳已填寫清單」Excel 檔也一併搬遷**（原本評估後排除在圖片搬遷範圍外，Jim 確認 Excel 檔案體積小、不會排擠圖片配額，但一樣需要版本歷史，決定另開 bucket 處理）：
  ...
  4. **已知限制，尚未做完整前端視覺驗證**：這次改動只驗證了 Storage／RLS 層（`download()`／`createSignedUrl()`／私有網址阻擋都正常），沒有做到端對端瀏覽器點擊測試——原因是（a）持有這兩筆資料的專案剛好沒有採購 AVA／GuestWeb，後台看不到對應分頁；（b）**這次改動的前端 commit 還沒部署**，正式站目前還是跑舊版程式碼，用舊邏輯讀新的私有路徑字串會顯示「無法預覽此檔案」且下載連結會 404（不會整頁掛掉，但預覽/下載暫時是壞的），要等 Jim push＋部署新版才會恢復正常。**建議 Jim 部署後找一個有採購 AVA 或 GuestWeb、且原本就有上傳過 amenity/repair 清單的專案，實際點開後台服務設定分頁確認預覽表格與下載按鈕都正常運作。**
